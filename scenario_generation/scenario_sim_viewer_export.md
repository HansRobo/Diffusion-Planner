...
closed_loop_scenario/
  <run>/                        # 評価実行 1 回。dataset 直下の 1 段だけ
    ...
    scenario_names.json         # スイート同梱の名前ファイルをそのままコピー
    media/
      <scenario_id>/            # Web.Auto のシナリオ ID（uuid）
        <case_key>.mp4
        <case_key>.rollout.jsonl
        <case_key>.<metric>.png
```
...
**同一シーンの突き合わせキーは `(scenario, case_key)`** です。`parameters` を突き合わせれば、
シナリオが改版されても同じ条件のケース同士を対応させられます（その判断は読む側が行います）。
...
```json
{
  "run_dir": "<run 出力のパス>",
  "ckpt": "<checkpoint のパス>",
  "dp_commit": "<commit>", "branch": "<branch>",
...
| フィールド | 用途 |
|---|---|
| `description` | シナリオ自身が書いている説明。読む側が uuid のままでは分からないので |
...
スイートが同梱している名前ファイルを**そのままコピー**したものです。エクスポータは中身を解釈
しません。`scenario_id` から表示名を引き、カテゴリの分け方や表示名も**読む側が決めてください**。
無いこともあります（スイートが同梱していないラン）。

## cases.jsonl

1 行 1 ケース。行のスキーマは npz 経路の `segments.jsonl` と同じで、
`scenario` / `case_key` / `parameters` が加わります。

```json
{
  "scenario": "<scenario_id>",
  "case_key": "<ケースの識別子。メディアのファイル名でもある>",
  "parameters": { "<パラメータ名>": "<値>" },
...
- 行はメディアのパスを持ちません。`media/<scenario>/<case_key>.mp4` と決まるので、
  読む側はその名前で引きます
- `parameters` は**そのケースの展開が設定した値**です。同一シナリオのケース同士を比べれば
  何が変わっているかが分かるので、**表示名の付け方は読む側が決めてください**
...
| `type` | `SimulationFailure` / `SyntaxError` など。`kind` より細かい失敗の種類 |
| `trigger` | 判定を引いた条件の文 |
...
```text
media/<scenario_id>/<case_key>.mp4
                    <case_key>.rollout.jsonl
                    <case_key>.<metric>.png
```
...
カラーマップ画像はエクスポート時に常に描きます（464 ケースで 2784 枚・262 秒）。

### 動画時刻

```
video_time[s] = step * draw_every / fps
```

`draw_every` sim step ごとに 1 フレーム、`fps` で符号化されています（例: 4 step ごと・10fps
なら `step / 40`）。**間引き幅は実行ごとに変わる**ので、定数として持たず
`run.json` の `draw_every` / `fps` から計算してください。エクスポータは動画を書き換えません
（生成物のバイト列はランのものと同一で、ハードリンクで共有されます）。
...
`<run 名>` は 1 階層にまとめます（例 `<日付>-<時刻>_<構成>`）。
