...
class _OnnxModel:
    ...
    def __init__(
        ...
        # itself is on the GPU, so the pool buys nothing to make up for it. It decides how the
        # host feeds the accelerator, not what is computed. SCENARIO_SIM_ORT_INTRA overrides it.
    ...
    def warm_up(self) -> None:
        """Run the graph once on zeros of its declared shapes.

        TensorRT builds its engine on the first run, not when the session opens, so this is what
        populates the engine cache. The dynamic batch axis is taken as 1, which is what the
        rollout feeds.
        """
        feed = {}
        for i in self.session.get_inputs():
            shape = [d if isinstance(d, int) else 1 for d in i.shape]
            feed[i.name] = np.zeros(shape, dtype=bool if i.type == "tensor(bool)" else np.float32)
        self.session.run(self._outputs, feed)
