...
one line per step, independent of ``instrument``) and draws a colored polyline
(risk metric -> color) with a colorbar legend, similar in spirit to a routing app's
"road risk" heatmap overlay.
...
def _risk_and_ticks(
    ...
    if metric == "near_miss":
        ...
        return risk, [0.0, 1.0], [f"clearance > {near_miss_thresh:.2f}m", f"clearance <= {near_miss_thresh:.2f}m"]
    if metric == "speed":
...
def render_trajectory_colormap(
    ...
    *,
    ...
    if metric in _BINARY_METRICS:
        ...
        if event_xy.size:
            ax.scatter(
                event_xy[:, 0], event_xy[:, 1], c="red", s=110, zorder=4, edgecolors="black", linewidths=0.8
            )
