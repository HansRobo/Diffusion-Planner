...
class SceneTraceWriter:
    ...
    def __init__(self, output_path: Path, *, map_ref: str, route: list[list[float]], goal: list[float]):
        self.output_path = output_path
        self._file = gzip.open(output_path, "wt", encoding="utf-8")
        self._agents: dict[str, dict[str, Any]] = {}
        self._header = {"event": "header", "version": SCENE_TRACE_VERSION, "map_ref": map_ref,
                        "route": route, "goal": goal, "agents": self._agents}

    def write_frame(self, step: int, scene, plan_world: np.ndarray, *, clearance: float | None, collision: bool | None) -> None:
        agents = []
        for agent in scene.agents:
            self._agents.setdefault(agent.id, {"type": str(agent.agent_type), "length": round(float(agent.length), 3),
                                                "width": round(float(agent.width), 3), "wheelbase": round(float(agent.wheelbase), 3)})
            x, y = agent.current_position
            agents.append([agent.id, round(float(x), 3), round(float(y), 3), round(float(agent.current_heading), 4)])
        frame: dict[str, Any] = {"k": step, "agents": agents,
                                 "plan": _points(plan_world), "clearance_m": None if clearance is None else round(clearance, 4),
                                 "collision": collision}
    ...
    def close(self, terminated_reason: str) -> None:
        ...
        self._file.write(json.dumps({**self._header, "terminated": terminated_reason}, separators=(",", ":"), ensure_ascii=False) + "\n")
        self._file.close()
