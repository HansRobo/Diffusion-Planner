    ...
    for i in range(n):
        ...
        for key, value in subscores.items():
            if torch.is_tensor(value):
                sample = value[i].detach().cpu()
                item[key] = float(sample.item()) if sample.numel() == 1 else sample.tolist()
...
def reward_breakdown_to_json_dict(breakdown: dict) -> dict:
    return _json_safe_value(breakdown)
...
class SpawnConfig:
    ...
    # Retained only so old replay JSON files with the field still load. Temporal HDP removed
    # delay-prefix inference, so validation requires the legacy value to remain zero.
    ...
    def validate(self) -> None:
        ...
        if self.inference_delay != 0:
            raise ValueError(
                "inference_delay is unsupported by temporal HDP and must be 0; "
                f"got {self.inference_delay}"
            )
        if self.max_steps < 1:
...
def _backfill_neighbor_futures(npz_dir: Path) -> None:
    ...
    for _fi, p in enumerate(npz_paths):
        d = np.load(p)
