...
def test_map_asset_is_deduplicated_and_trace_is_valid_jsonl(tmp_path):
    ...
    trace = SceneTraceWriter(tmp_path / "case.scene.jsonl.gz", map_ref=ref, route=[[0, 0]], goal=[10, 0])
    agent = type("Agent", (), {"id": "ego", "agent_type": "VEHICLE", "length": 4.0, "width": 1.8, "wheelbase": 2.6, "current_position": np.array([1.0, 2.0]), "current_heading": 0.1})()
