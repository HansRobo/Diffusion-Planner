...
def _site_color(index: int) -> str:
    return _PALETTE[index % len(_PALETTE)]


def collect_site_data(
    out_root: str | Path, site_names: list[str], *, colormap_metrics: tuple[str, ...] = METRIC_CHOICES
        ...
        with segments_path.open(encoding="utf-8") as f:
            for line in f:
                ...
                missing_metrics = tuple(
                    m for m in colormap_metrics if not (site_dir / f"{stem}_trajcolormap_{m}.png").is_file()
                )
                if missing_metrics:
                    try:
                    ...
                    except Exception as e:  # pragma: no cover - a bad episode must not break the report
...
<h2>エピソード一覧</h2>
<div class="controls">
  <input type="text" id="search" placeholder="routeで検索">
