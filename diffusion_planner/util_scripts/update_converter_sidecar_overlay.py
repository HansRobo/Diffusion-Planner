...
import contextlib  # noqa: E402  (import follows the sys.path bootstrap above)
...
def main() -> None:
    ...
    with executor_type(max_workers=args.workers) as executor:
        for source in args.source:
            ...
            # source and checkpoint_root are bound as defaults rather than captured: the
            # closure is redefined per loop iteration, so a late call would otherwise read
            # whichever source the loop had reached instead of the one it was built for.
            def flush(source=source, checkpoint_root=checkpoint_root) -> None:
