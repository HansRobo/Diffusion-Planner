...
class TrainConfig(ClosedLoopConfig, ScenarioOpenLoopConfig, ModelConfig):
    ...
    unknown_class_rename_prob: float = cli(
        "per-agent probability of relabeling a valid neighbor's type one-hot to Unknown "
        "during training, regardless of its original class; 0.0 is a true no-op that "
        "draws no random numbers",
        default=0.0,
    )
    unknown_rename_debug_dir: str = cli(
        "confirmation tooling for unknown_class_rename_prob (see "
        "utils/unknown_rename_debug.py): the per-step rename count/rate is always "
        "logged; setting this directory additionally dumps one before/after PNG per "
        "epoch, on that epoch's last training step. Empty = no images",
        default="",
        path=True,
    )
    ...
    neighbor_collision_margin_unknown: float = cli(
        "collision margin (m) for Unknown-typed neighbors. Unknown could be anything, "
        "including a pedestrian, so this defaults conservatively to the pedestrian "
        "margin and treats Unknown as a VRU (see neighbor_db.py)",
        default=1.0,
    )
