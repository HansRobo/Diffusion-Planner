...
def closed_loop_validate(model, args, epoch: int, out_dir: str) -> None:
    ...
    def run_one(
        npz_root: str, site_out_dir: str, site_name: str | None, drop_objects: bool = False
    ) -> tuple[dict, dict]:
    ...
    try:
        if args.closed_loop_sites_root:
            ...
            # Each site runs once per object-mode. "noobj" gets a distinct label (suffix) rather
            # than a separate axis, so it rides the existing per-site machinery (per-site wandb
            # keys, metric_regex overlay, combined episode table) unchanged — see
            # objects_ablation_strategy.md §5. Doubles closed-loop cost per firing (accepted).
            object_modes = (("objects", False), ("noobj", True))
            for site_name, npz_root in sites.items():
                for tag, drop_objects in object_modes:
                    label = site_name if tag == "objects" else f"{site_name}__noobj"
                    site_out_dir = os.path.join(out_dir, label)
                    site_log, summary = run_one(
                        str(npz_root), site_out_dir, label, drop_objects=drop_objects
                    )
                    if summary and _is_grouped_closed_loop_summary(summary):
                        ...
                        log.update(
                            {
                                key.replace("closed_loop/", f"closed_loop/{label}/", 1): val
                        ...
                        log.update(site_log)
                        if summary:
                            site_summaries[label] = summary
                            episode_data.append((label, summary.get("segments") or [], site_out_dir))
