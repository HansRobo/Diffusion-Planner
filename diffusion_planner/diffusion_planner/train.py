...
def model_training(args: TrainConfig):
    ...
    if global_rank == 0:
        ...
        mean_traj_metric_dict = {f"valid_traj/{k}": v for k, v in agg["traj_means"].items()}
        ...
        for key, value in mean_traj_metric_dict.items():
            print(f"{key}={value:.4f}")
    ...
    for epoch in range(init_epoch, train_epochs):
        ...
        # (The old "final 10 epochs" manual lr step-down was removed: the lr
        # schedule now does a proper cosine decay to eta_min, so a second manual
        # decay would double-count. See utils/lr_schedule.py.)
        ...
        if global_rank == 0:
            ...
            curr_data = {
                ...
                **{k.replace("/", "_"): v for k, v in mean_traj_metric_dict.items()},
                **replan_agg,
