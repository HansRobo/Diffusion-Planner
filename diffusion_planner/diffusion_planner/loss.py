...
# The one action space every conversion goes through, so the GT side and the loss side
# cannot drift into different unicycle parameters.
ACTION_SPACE = UnicycleAccelCurvatureActionSpace(n_waypoints=OUTPUT_T)
...
def compute_control_traj_loss(
    ...
    accel = ctrl_denorm[..., 0] * ACTION_SPACE.accel_std.to(device) + ACTION_SPACE.accel_mean.to(
        device
    )
    kappa = ctrl_denorm[..., 1] * ACTION_SPACE.curvature_std.to(
        device
    ) + ACTION_SPACE.curvature_mean.to(device)
