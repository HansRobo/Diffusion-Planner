...
def _sft_step(raw_inputs, model, optimizer, args, ema):
    ...
    (see ``DiffusionPlannerData``), so every batch arrives perturbed and recentred.
    """
...
def _grpo_step(raw_inputs, model, optimizer, args, ema, collider_injector):
    ...
    The scene arrives already perturbed and recentred by the DataLoader worker (see
    ``DiffusionPlannerData``); collider injection therefore targets the perturbed future and the
    policy is sampled / gt-L2-scored on the perturbed scene, as before.
    """
    ...
    # Idempotent: the perturbation in the DataLoader worker already emits cos/sin form.
