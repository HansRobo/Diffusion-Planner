...
class TrainConfig:
    ...
    # multi-site validation: each direct subdirectory of the root (e.g. one per {project}/{map}/)
    # is auto-discovered (see scenario_generation.site_discovery) and evaluated as its own
    # independent npz_root, wandb-logged under "closed_loop_scores/<metric>/<site_name>". Both may
    # be set at once — each fires independently and contributes its own rows to the combined
    # episode table / cross-site aggregate.
    # ---------------------------------------------------------
    closed_loop_npz_root: str = ""
    closed_loop_sites_npz_root: str = ""
    # Object-mode ablation per source (see objects_ablation_strategy.md): "objects"=normal,
    # "noobj"=empty-world (no dynamic/static objects, map kept — isolates "reacts badly to
    # traffic" from "can't follow the route/map"). npz_root defaults to objects-only (usually a
    # single curated scene); sites_root defaults to both (the objects-vs-noobj comparison).
