...
class LineEncoder(nn.Module):
    def __init__(self, line_len, class_type, drop_path_rate, hidden_dim, depth, num_types):
        ...
        self.channel_pre_project = Mlp(
            in_features=4,  # x, y, dx, dy; the line type is injected after pooling
            hidden_features=channels_mlp_dim,
            out_features=channels_mlp_dim,
            act_layer=nn.GELU,
            drop=0.0,
    ...
    def forward(self, x):
        ...
        # Keep the categorical line type out of the geometric mixer.  It is added
        # once after pooling, as in the upstream LineEncoder refactor.
        type_one_hot = x[:, :, 0, 2:]
        feat = torch.concat([x[..., :2], diff_x, diff_y], dim=-1)

        valid = valid_pt.to(feat.dtype).unsqueeze(-1)
        count = valid.sum(dim=2).clamp(min=1.0)
        center_xy = (feat[..., :2] * valid).sum(dim=2) / count
        mean_diff = (feat[..., 2:4] * valid).sum(dim=2) / count
