...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        if cond is None:
            if not scalar_time:
                # The real-time-chunking DiT wants the trajectory as (B, P, T, 4); with a
                # scalar timestep it wants the flat (B, P, T*4) the solver already holds.
...
class DPM_Solver:
    ...
    def multistep_dpm_solver_second_update(self, x, model_prev_list, t_prev_list, t):
        ...
        phi_1 = torch.expm1(-h)
        if not self._scalar_time:
            # Real-time chunking's model output is (B, P, T, 4); a scalar-time model
