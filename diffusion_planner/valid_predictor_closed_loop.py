...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=4,
        help="render the PNGs on this many worker processes instead of in the rollout loop (0 = in-loop). Output is byte-identical; costs ~780 MB of RSS per worker, per GPU shard",
    )
    p.add_argument(
        "--abort_deviation_m",
        type=float,
