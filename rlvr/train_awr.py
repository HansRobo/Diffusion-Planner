...
Examples:
    ...
    python -m rlvr.train_awr \
      ...
      --train_npz_list datasets/20260623_full_sequence/path_list_train_sft.json \
      --valid_npz_list datasets/20260623_full_sequence/path_list_valid_sft_balanced.json \
...
from rlvr.posttrain import objectives as posttrain_objectives
...
def _load_config(path: Path) -> tuple[dict[str, Any], AWRRolloutConfig, RewardConfig]:
    ...
    rollout = AWRRolloutConfig(
        ...
        objective=str(awr_raw.get("objective", "awr")),
...
def apply_reward_overrides(
    reward_config: RewardConfig, items: list[str] | None
) -> dict[str, Any]:
    """Set any RewardConfig field from ``key=value`` strings, coerced by its
    current type.  An unknown key is an error: a silently ignored reward
    override is a silent method change."""

    applied: dict[str, Any] = {}
    known = {field.name for field in fields(RewardConfig)}
    for item in items or []:
        key, separator, text = str(item).partition("=")
        if not separator or key not in known:
            raise ValueError(
                f"--reward_override expects a known key=value, got {item!r}"
            )
        current = getattr(reward_config, key)
        if isinstance(current, bool):
            parsed: Any = text.strip().lower() in ("1", "true", "yes", "on")
            ...
            parsed = text
        setattr(reward_config, key, parsed)
        applied[key] = parsed
    return applied
...
def _repo_path(value: str) -> Path:
    """Config paths are project-relative, so a config is not tied to a host."""
...
def _trajectory_metrics(trajectory: torch.Tensor, data: dict[str, torch.Tensor]) -> dict[str, float]:
    ...
    if gt.dim() == 3:
        gt = gt[0]
    gt = gt.float()
    n = min(gt.shape[0], trajectory.shape[0])
    if gt.dim() != 2 or gt.shape[-1] < 2 or n < 1:
        return result
    gt, pred = gt[:n], trajectory[:n]
    # Padding is a TRAILING property, so it is cut as a prefix length and the pairing
    # stays index-to-index.  The mask this replaces (``|x|+|y| > 0.1`` followed by
    # ``gt_xy[valid]``) compacted instead, so a near-origin logged row anywhere slid every
    # later one forward and pred step i was compared against the i-th SURVIVING logged
    # step.  |x|+|y| = 0.1 m is ~1 m/s at 10 Hz, so it fired on standstill scenes and
    # nowhere else: over 40,000 mine rows it shifted 9.91% of scenes, inflating their ade
    # by +0.73 m mean and the 0-0.5 m/s band by 60% (1.6354 -> 2.6201 m), and it dropped
    # ade/fde entirely on another 9.28%.  It also mislabelled the data -- an exactly-zero
    # logged future comes with an all-zero ego current state, i.e. a parked ego, and only
    # 10 rows in 40,000 have a *partial* trailing zero run.  So a fully stationary future
    # is scored (how far the model creeps while the human holds still is the whole point
    # at standstill) and only a partial trailing run counts as padding.  mean_ade now
    # averages over ~all scenes instead of 90.7% of them, so it is not comparable with
    # any epoch evaluated before this.
    zero = ~gt[:, :2].abs().any(dim=-1)
    if bool(zero.any()) and not bool(zero.all()):
        keep = int(torch.nonzero(~zero)[-1].item()) + 1
    ...
    result["gt_path_len"] = (
        float(torch.diff(gt_xy, dim=0).norm(dim=-1).sum().item())
        if gt_xy.shape[0] > 1
        else 0.0
    )
    # The executed waypoint against the logged human.  One frame is one plan step, so the
    # vehicle drives index 0 and the next replan discards the rest -- and nothing else
    # emitted here is about it: ade averages it with 79 steps that are never driven, and
    # det_progress is anti-correlated with it per scene.  The anchor arms' entire
    # justification was therefore only readable from an offline re-score; these make it a
    # read on scenes.json, per epoch, at no cost.  Signed positions as well as the errors,
    # because the model's step-0 lateral defect is a BIAS (+4.972 mm against 1.739 mm of
    # dispersion) and an absolute value cannot show a bias closing.  Per-scene only:
    # nothing here is added to the aggregated dict, which is reduced across 8 ranks.
    result["det_exec_lon_m"] = float(pred_xy[0, 0].item())
    result["det_exec_lat_m"] = float(pred_xy[0, 1].item())
    result["gt_exec_lon_m"] = float(gt_xy[0, 0].item())
    result["gt_exec_lat_m"] = float(gt_xy[0, 1].item())
    result["det_exec_lon_err_m"] = float((pred_xy[0, 0] - gt_xy[0, 0]).abs().item())
    result["det_exec_lat_err_m"] = float((pred_xy[0, 1] - gt_xy[0, 1]).abs().item())
    result["det_exec_err_m"] = float((pred_xy[0] - gt_xy[0]).norm().item())
    if pred.shape[-1] >= 4 and gt.shape[-1] >= 3:
        # The plan carries heading as (cos, sin); the logged future carries the angle.
        # This is the axis the anchor actually spends itself on -- 95.8% of its gradient
        # in normalized space -- so it is the one that says whether the anchor is working
        # or only moving position.
        delta = torch.atan2(pred[0, 3], pred[0, 2]) - gt[0, 2]
        result["det_exec_heading_err_rad"] = float(
            torch.atan2(torch.sin(delta), torch.cos(delta)).abs().item()
        )
    return result
    ...
    for batch_start, batch_paths, model_batch_paths, prefetched_data in prepared_batches:
        ...
        for offset, (scene_path, rollout) in enumerate(
            ...
            row: dict[str, Any] = {
                ...
                # The step behind each of the two crossing booleans, so a
                # narrower window is a threshold on scenes.json instead of a
                # GPU re-score.  Both flags above are steps-1..40 verdicts
                # (``compute_reward_batch`` truncates to
                # ``reward_horizon_steps`` before scoring), so these are None
                # for a crossing that first happens in the unscored tail --
                # 36% of them.  Per-scene only, never folded into the
                # aggregated metrics dict: that dict is reduced across 8 DDP
                # ranks and a None has no reduction.
                "det_rb_crossing_step": det.rb_crossing_step,
                "det_lane_crossing_step": det.lane_crossing_step,
...
def build_parser() -> argparse.ArgumentParser:
    """The full flag surface.  rlvr.posttrain validates specs against it."""
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "candidate-weighting objective: "
            + " | ".join(posttrain_objectives.available())
        ),
    )
    parser.add_argument(
        "--awr_beta",
    ...
    parser.add_argument(
        ...
        action="append",
        metavar="KEY=VALUE",
        help="set any RewardConfig field, repeatable",
    )
...
def main() -> None:
    ...
    reward_overrides = apply_reward_overrides(reward_config, args.reward_override)
    ...
    if args.awr_objective is not None:
        rollout_config.objective = args.awr_objective
    posttrain_objectives.resolve(rollout_config.objective)
    ...
    if is_main:
        ...
        # Resolve the two candidate-loss knobs to the values the loss actually uses.  They
        # only reach ``train_config`` when passed explicitly, so an arm running the default
        # wrote nothing and "absent" could not be told from "0" without reading the source --
        # which is the one distinction a candidate-start A/B turns on.  Same defaults as the
        # consumers above.
        effective_config["training"]["awr_candidate_loss_horizon"] = int(
            train_config.get("awr_candidate_loss_horizon", 0)
        )
        effective_config["training"]["awr_candidate_loss_start"] = int(
            train_config.get("awr_candidate_loss_start", 0)
        )
        # Which checkpoint, which seed, which epoch span.  ``provenance.json`` carries the
        # staged model and its sha256; these are the fields that decide whether two run dirs
        # are comparable at all, and none of them were recoverable from the artifacts.
        effective_config["run_identity"] = {
        ...
        _write_json(
            run_dir / "provenance.json",
            {
                ...
                "rank": rank,
                "awr_objective": rollout_config.objective,
                ...
                "plannerrft_reference_model": (
                    str(_repo_path(plannerrft_config["reference_model_path"]))
                    if rollout_config.plannerrft_guided_exploration
                    else None
                ),
                "plannerrft_reference_args": (
                    str(_repo_path(plannerrft_config["reference_args_path"]))
                    if rollout_config.plannerrft_guided_exploration
                    else None
                ),
    ...
    if rollout_config.plannerrft_guided_exploration:
        ...
        reference_model_path = _repo_path(plannerrft_config["reference_model_path"])
        reference_args_path = _repo_path(plannerrft_config["reference_args_path"])
