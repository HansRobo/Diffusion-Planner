...
# Independent R2LPL-style mistake-priority ablation on the immutable
# PlannerRFT-enriched cache.  Hard is an auditable cache-local proxy:
# final best reward < 0.925 and at least one non-zero AWR target.  Half of
# every batch remains ordinary replay to retain broad behavior.
...
CACHE=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_enriched_unique153600_cycle_pilot/20260720-010747_plannerrft_enriched_unique153600_cycle_pilot/replay_buffer
OUT=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_enriched_unique153600_priority_replay_pilot
...
exec "${ROOT}/.venv/bin/torchrun" --standalone --nproc_per_node=8 \
  ...
  --config "${CONFIG}" \
  --output_dir "${OUT}" \
  --exp_name plannerrft_enriched_unique153600_priority50_replay_pilot \
  --device cuda \
  --seed 3407 \
  --start_epoch 2 \
  --epochs 2 \
  --resume_replay_root "${CACHE}" \
  --replay_sampling reward_stratified \
  --replay_priority_best_reward_lt 0.925 \
  --replay_priority_fraction 0.5 \
