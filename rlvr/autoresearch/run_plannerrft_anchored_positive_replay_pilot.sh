...
# Positive-advantage AWR on the immutable behavior-anchored PlannerRFT cache.
# Only candidates whose reward strictly exceeds candidate-0 behavior are
# trained, with candidate 0 retained as an explicit behavior anchor.  The
# overlay changes weights only; trajectories/rewards/encodings are bit-identical.
...
CACHE=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_anchored_positive_overlay_beta1/replay_buffer
OUT=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_anchored_positive_replay_pilot
...
exec "${ROOT}/.venv/bin/torchrun" --standalone --nproc_per_node=8 \
  ...
  --config "${CONFIG}" \
  --output_dir "${OUT}" \
  --exp_name plannerrft_anchored_positive_replay_pilot \
  --device cuda \
  --seed 3407 \
  --start_epoch 2 \
  --epochs 2 \
