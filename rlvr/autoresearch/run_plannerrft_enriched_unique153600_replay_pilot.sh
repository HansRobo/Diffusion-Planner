...
OUT=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_enriched_unique153600_replay_pilot
...
# B=96 x accumulation=2 keeps the original effective 192-scene optimizer
# batch and one EMA update per effective batch while halving backward VRAM.
exec "${ROOT}/.venv/bin/torchrun" --standalone --nproc_per_node=8 \
  ...
  --config "${CONFIG}" \
  --output_dir "${OUT}" \
  --exp_name plannerrft_enriched_unique153600_replay_pilot \
  --device cuda \
  --seed 3407 \
  --start_epoch 2 \
  --epochs 2 \
