...
"WARNING: " + ("decoder-context scene order/hash differs from replay paths: "
            ...
            if has_expert_sidecar:
                ...
                # A step-scoped sidecar carries a fourth array and the shipped
                # arm requires it; every array is linked through below, so
                # demand these three rather than exactly these three.
                required_expert_arrays = {
                    "expert_trajectories",
                    "expert_rewards",
                    "expert_safe",
                }
                if not required_expert_arrays <= set(expert_arrays):
                    raise RuntimeError(
