...
# Matched-cache ablation for the 153,600-scene PlannerRFT enrichment pilot.
# The only semantic difference from the reference replay pilot is one
# shuffled, exhaustive pass instead of ReplayBuffer-style sampling with
# replacement.  This isolates one-pass sampling variance before any hard-case
# prioritisation is introduced.
...
OUT=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_enriched_unique153600_shuffle_replay_pilot
...
exec "${ROOT}/.venv/bin/torchrun" --standalone --nproc_per_node=8 \
  ...
  --config "${CONFIG}" \
  --output_dir "${OUT}" \
  --exp_name plannerrft_enriched_unique153600_shuffle_replay_pilot \
  --device cuda \
  --seed 3407 \
  --start_epoch 2 \
  --epochs 2 \
