...
# One exhaustive replay pass for the behavior-anchored PlannerRFT cache.
# This first causal update avoids with-replacement coverage variance: every
# mined scene group is consumed exactly once in a shuffled order.
...
OUT=${ROOT}/outputs/awr_t4_full_sequence_filtered/plannerrft_anchored_unique153600_shuffle_replay_pilot
...
exec "${ROOT}/.venv/bin/torchrun" --standalone --nproc_per_node=8 \
  ...
  --config "${CONFIG}" \
  --output_dir "${OUT}" \
  --exp_name plannerrft_anchored_unique153600_shuffle_replay_pilot \
  --device cuda \
  --seed 3407 \
  --start_epoch 2 \
  --epochs 2 \
