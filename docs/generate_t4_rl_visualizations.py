...
def plot_rl_diagnostics() -> None:
    ...
    ess = [
        float(g8["train_reward_weight_ess_fraction"]),
        float(g32["train_reward_weight_ess_fraction"]),
    ]
    top1 = [
        float(g8["train_reward_weight_top1_share"]),
        float(g32["train_reward_weight_top1_share"]),
    ]
    xx = np.arange(2)
    ...
    style_axes(ax)
    for bar, value in zip(bars, max_w, strict=False):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            value * 1.12,
            f"{value:.2f}×",
    ...
    selection = [float(baseline["selection_score"])] + [
        float(row["valid_selection_score"]) for row in g32_rows
    ]
    ...
    for x, yv in zip(epochs, selection, strict=False):
        ax.annotate(
            f"{yv:.4f}", (x, yv), textcoords="offset points", xytext=(0, 8), ha="center", fontsize=8
        )
...
def plot_data_composition() -> None:
    ...
    fig.suptitle(
        "Local Tier-IV/T4 manifest composition", fontsize=16, fontweight="bold", color="#102a43"
    )
    ...
    ax.text(
        0.02,
        0.03,
        "Counts read from artifacts/rl_train_real/base_uniform_100000.json",
        transform=ax.transAxes,
        fontsize=8,
        color="#52606d",
    )
    ...
    ax.text(
        0.02,
        0.03,
        "The full valid manifest contains 53,382 unique clips; this is its uniform audit sample.",
        transform=ax.transAxes,
        fontsize=8,
        color="#52606d",
    )
...
def plot_metric_gallery() -> None:
    ...
    def panel_title(ax: plt.Axes, title: str, subtitle: str) -> None:
        ...
        ax.text(
            0.0, 1.01, subtitle, transform=ax.transAxes, fontsize=8, color="#52606d", va="bottom"
        )
        style_axes(ax)

    def bar_panel(
        ax: plt.Axes, labels: list[str], vals: list[float], colors: list[str], *, xmax: float = 1.0
    ) -> None:
        y = np.arange(len(labels))
        bars = ax.barh(y, vals, color=colors, edgecolor="white")
        ax.set_yticks(y, labels)
    ...
    ax = axes[0, 0]
    panel_title(
        ax, "A · Collision and safety gate", "score: higher is better; rates: lower is better"
    )
    labels = ["collision safety", "behavior gate", "active collision rate", "rear collision rate"]
    vals = [
        value("valid_reward_collision_safety"),
        value("valid_reward_behavior_gate"),
        value("valid_reward_collision_active"),
        value("valid_reward_collision_rear"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "A gate is not the same as a collision count: the gate summarizes whether safety dominates behavior.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    ax = axes[0, 1]
    panel_title(
        ax, "B · TTC, THW, and risk", "TTC/THW are time margins; risk is the aggregate risk score"
    )
    labels = [
        "TTC score",
        "THW score",
    ...
    vals = [
        value("valid_reward_ttc"),
        value("valid_reward_thw"),
        value("valid_reward_risk"),
        value("valid_reward_ttc_zero_fraction"),
        value("valid_reward_thw_zero_fraction"),
        value("valid_reward_risk_zero_fraction"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "TTC asks ‘when would we collide if closing continues?’; THW asks ‘how many seconds of gap?’",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    ax = axes[0, 2]
    panel_title(
        ax, "C · Occupancy source breakdown", "source fractions in the same validation audit"
    )
    labels = ["stopped neighbor", "road-border fallback", "static object", "occupancy zero"]
    vals = [
        value("valid_reward_occupancy_stopped_source"),
        value("valid_reward_occupancy_road_border_source"),
        value("valid_reward_occupancy_static_source"),
        value("valid_reward_occupancy_zero_fraction"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "This is why border clearance must be reported separately from object occupancy.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    panel_title(
        ax,
        "D · Red light and road border",
        "hard geometry constraints should not be hidden in a scalar reward",
    )
    labels = [
        "red-light score",
        "road-border score",
        "red constraint active",
        "red violation",
        "border violation",
    ]
    vals = [
        value("valid_reward_red_light"),
        value("valid_reward_road_border"),
        value("valid_reward_red_light_constraint_active_fraction"),
        value("valid_reward_red_light_violation_fraction"),
        value("valid_reward_road_border_violation_fraction"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "A 0.989 red-light score still needs its 1.06% violation fraction shown.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    panel_title(
        ax,
        "E · Behavior quality",
        "higher is usually better, but progress must be read with safety and stop-state slices",
    )
    labels = ["progress", "comfort", "lane", "stationary progress", "follow"]
    vals = [
        value("valid_reward_progress"),
        value("valid_reward_comfort"),
        value("valid_reward_lane"),
        value("valid_reward_stationary_progress"),
        value("valid_reward_follow"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "follow is conditional on a leader; stationary progress is a separate mode for stopped scenes.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    ax = axes[1, 2]
    panel_title(
        ax, "F · EPDMS / held-out planner score", "external-style guard metrics; higher is better"
    )
    epdms = [
    ...
    bar_panel(
        ...
        [x[0] for x in epdms],
        [x[1] for x in epdms],
        ["#2d9cdb", "#4c956c", "#f2c94c", "#d64550", "#829ab1", "#ef8354"],
        xmax=1.05,
    )
    ...
    ax.text(
        0.02,
        0.02,
        "EPDMS is a promotion guard, not the same object as the training reward.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    panel_title(
        ax,
        "G · Reward correlation: what is learnable?",
        "correlation with total reward; not a quality score",
    )
    labels = ["progress", "lane", "follow", "risk", "safety"]
    vals = [
        value("valid_reward_progress_correlation"),
        value("valid_reward_lane_correlation"),
        value("valid_reward_follow_correlation"),
        value("valid_reward_risk_correlation"),
        value("valid_reward_safety_correlation"),
    ]
    ...
    ax.text(
        0.02,
        0.02,
        "The local total reward is much more rankable by progress/lane than by safety/risk.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    panel_title(
        ax,
        "H · weight / ESS / update health",
        "G=8 is the conservative local reference; G=32 is the concentrated arm",
    )
    g8 = read_tsv(G8_LOG)[0]
    names = ["G=8 max weight", "G=32 max weight", "G=8 ESS", "G=32 ESS", "G=8 top-1", "G=32 top-1"]
    vals = [
        float(g8["train_reward_weight_max"]),
        float(row["train_reward_weight_max"]),
        float(g8["train_reward_weight_ess_fraction"]),
        float(row["train_reward_weight_ess_fraction"]),
        float(g8["train_reward_weight_top1_share"]),
        float(row["train_reward_weight_top1_share"]),
    ]
    ...
    for bar, val in zip(bars, vals, strict=False):
        ax.text(
            float(val) + (0.15 if val < 2 else 1.5),
            bar.get_y() + bar.get_height() / 2,
    ...
    ax.text(
        0.02,
        0.02,
        "ESS = effective sample size; lower ESS means fewer candidates carry the update.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
    ...
    panel_title(
        ax,
        "I · source baseline → final row",
        "small changes are why we need source-relative promotion guards",
    )
    pairs = [
    ...
    ax.text(
        0.02,
        0.02,
        "No claim of improvement is made unless the pre-registered guards pass.",
        transform=ax.transAxes,
        fontsize=7.5,
        color="#52606d",
    )
...
def plot_red_light_audit() -> None:
    ...
    categories = [
        "red→green before motion",
        "red at onset; no crossing",
        "green after motion",
        "ambiguous",
        "confirmed crossing",
    ]
    ...
    ax.set_title(
        "Causal red-light audit: the old filter removed 2,051 clips, but found zero confirmed crossings",
        fontsize=14,
        fontweight="bold",
        color="#102a43",
        loc="left",
    ...
    for bar, val in zip(bars, counts, strict=False):
        ax.text(
            max(val + 5, 5), bar.get_y() + bar.get_height() / 2, str(val), va="center", fontsize=9
        )
    ax.text(
        0.02,
        0.02,
...
def draw_polyline(
    ax: plt.Axes, arr: np.ndarray, color: str, lw: float, alpha: float, boundaries: bool = False
) -> None:
    ...
    if mask.sum() < 2:
        return
    ax.plot(
        xy[mask, 0], xy[mask, 1], color=color, linewidth=lw, alpha=alpha, solid_capstyle="round"
    )
    ...
    for index, raw_path in enumerate(paths[:1000]):
        ...
        try:
            ...
            static = int(
                np.sum(
                    np.any(
                        np.abs(data.get("static_objects", np.zeros((0, 2)))[:, :2]) > 1e-4, axis=1
                    )
                )
            )
...
def draw_scene(ax: plt.Axes, data: dict[str, np.ndarray], frame: int | None = None) -> None:
    ...
    if nf.ndim == 3 and nf.shape[1] > 0:
        ...
        for idx in active:
            ...
            if np.any(np.abs(nf[idx, t, :2]) > 1e-4):
                ax.scatter(
                    nf[idx, t, 0], nf[idx, t, 1], s=13, color="#e67300", alpha=0.85, zorder=8
                )
    ...
    length, width = (
        float(shape[1] if len(shape) > 1 else shape[0]),
        float(shape[2] if len(shape) > 2 else shape[1]),
    )
    rect = Rectangle(
        (ex - length / 2, ey - width / 2),
        length,
        width,
        angle=np.degrees(eyaw),
        facecolor="#2d9cdb",
        edgecolor="#102a43",
        alpha=0.85,
        zorder=12,
...
def render_t4_scene() -> dict[str, str | int]:
    ...
    ax.set_title(
        "Logged Tier-IV/T4 scene used for RL training",
        fontsize=15,
        fontweight="bold",
        color="#102a43",
        loc="left",
    ...
    ax.text(
        0.01,
        0.01,
        f"manifest index {manifest_index}; {path.name}; lanes=gray/blue, border=red, stop line=yellow, neighbors=orange",
        transform=ax.transAxes,
        fontsize=8,
        color="#52606d",
    )
    ...
    active = (
        np.where(np.any(np.abs(nf[..., :2]) > 1e-4, axis=(1, 2)))[0]
        if nf.ndim == 3
    ...
    def update(frame: int):
        ...
        ax.set_aspect("equal", adjustable="box")
        ax.set_title(
            f"T4 logged dynamics — t={frame * 0.1:.1f}s",
            fontsize=13,
            fontweight="bold",
            color="#102a43",
        )
        ...
        ax.text(
            0.01,
            0.01,
            "logged future replay; orange agents are recorded neighbors",
            transform=ax.transAxes,
            fontsize=8,
            color="#52606d",
        )
    ...
    return {
        ...
        "frames": frames,
        "active_neighbors_plotted": int(len(active)),
    }
